...
abstract class LruMap<K, V> implements Map<K, V> {
  ...
  factory LruMap({int? maximumSize, Future<void> Function({required K key, required V value})? onEvicted}) =
  ...
  abstract final Future<void> Function({required K key, required V value})? onEvicted;
}
...
class LinkedLruHashMap<K, V> implements LruMap<K, V> {
  ...
  factory LinkedLruHashMap({int? maximumSize, Future<void> Function({required K key, required V value})? onEvicted}) =>
  ...
  LinkedLruHashMap._fromMap(this._entries, {int? maximumSize, this.onEvicted})
  ...
  @override
  final Future<void> Function({required K key, required V value})? onEvicted;
  ...
  @override
  void clear() {
    for (final element in _entries.values) {
      onEvicted?.call(key: element.key, value: element.value);
    }
    _entries.clear();
    _head = _tail = null;
  }
  ...
  @override
  Iterable<MapEntry<K, V>> get entries => _entries.values.map((entry) => MapEntry<K, V>(entry.key, entry.value));
  ...
  V putIfAbsent(K key, V Function() ifAbsent) {
    final evictedEntry = _entries[key];
    evictedEntry?.let((it) {
      onEvicted?.call(key: it.key, value: it.value);
    });
  ...
  V? remove(Object? key) {
    ...
    onEvicted?.call(key: entry.key, value: entry.value);
  ...
  void _insertMru(_LinkedEntry<K, V> entry) {
    ...
    final evictedEntry = _entries[entry.key];
    evictedEntry?.let((it) {
      onEvicted?.call(key: it.key, value: it.value);
    });
...
//
// @override
// Future<void> onEvicted(K key, V value) async {}
}
